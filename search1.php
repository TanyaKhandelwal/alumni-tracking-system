...
border-bottom:1px solid #dddddd;
    ...
    <input type="button" value="Reset" onClick = "fun()"/>
        ...
        <form action="search1.php" id="query_form" method="POST">
    ...
    $res=mysqli_query($db,"SELECT * FROM collegelogin where college_name='$a'");

    if(mysqli_num_rows($res)){
    $rey=mysqli_fetch_array($res);
    $cname =  $rey['college_name'];


    if(isset($_POST['sub1'])){
        $r=$_POST['rno'];
        $sql="SELECT * from alumni where rno LIKE '%$r%' and college = '$cname' and verified=1";
        $rec=mysqli_query($db,$sql);
        $temp=1;
    }
    if(isset($_POST['sub2'])){
        $r=$_POST['name'];
        $sql="SELECT * from alumni where name LIKE '%$r%' and college = '$cname' and verified=1";
        $rec=mysqli_query($db,$sql);
        $temp=1;
    ...
    if(isset($_POST['sub3'])){
        $r=$_POST['year'];
        $sql="SELECT * from alumni where Year(year) = '$r' and college = '$cname' and verified=1";
        $rec=mysqli_query($db,$sql);
        $temp=1;
